...
class Model1Dataset(Dataset):
    def __init__(self, npz_path, window = 192, assets = ASSETS):
        self.window = window
        self.assets = assets
        self.N = len(assets)
        ...
        assert self.regime_scores.shape == (self.T, 4)
...
class Model2Dataset(Dataset):
    ...
    def __init__(self, npz_path, seq_len_4h = 72, seq_k = 8, split = "train",
                 stride = 4, train_frac = 0.70, val_frac = 0.15,
                 lookback_15m = 96, lookback_1h = 48,
                 assets = ASSETS, bar_range = None):
        super().__init__()
        ...
        self.F_sentiment = self.sentiment_scores.shape[2]
        assert self.features_4h.shape == (self.T_4h, self.N, self.F_4h)
        assert self.targets.shape == (self.T_4h, self.N, 3)
        assert self.model1_outputs.shape == (self.T_4h, self.F_model1)
        assert self.T_4h > seq_len_4h + seq_k, "fewer 4h bars than seq_len + seq_k"
        self.asset_vol = self.targets[:, :, 0].std(axis = 0).astype(np.float32)
        all_starts = np.arange(seq_len_4h, self.T_4h - seq_k - 1, stride)
    ...
    def __getitem__(self, idx):
        ...
        f4h = np.stack([self.features_4h[t - L + 1 : t + 1] for t in ts])
        te4h = np.stack([self.time_enc_4h[t - L + 1 : t + 1] for t in ts])
        f15m_list, te15m_list, f1h_list, te1h_list = [], [], [], []
        for t in ts:
            ...
            s1h, e1h = int(self.window_idx_1h[t, 0]), int(self.window_idx_1h[t, 1])
            f1h_list.append(self._pad_to(self.features_1h, s1h, e1h, self.lookback_1h))
            te1h_list.append(self._pad_to(self.time_enc_1h, s1h, e1h, self.lookback_1h))
        return {
            "features_4h": torch.from_numpy(f4h),
            "time_enc_4h": torch.from_numpy(te4h),
            "features_15m": torch.from_numpy(np.stack(f15m_list)),
            "time_enc_15m": torch.from_numpy(np.stack(te15m_list)),
            "features_1h": torch.from_numpy(np.stack(f1h_list)),
            "time_enc_1h": torch.from_numpy(np.stack(te1h_list)),
    ...
    def get_current_sample(self):
        ...
        L = self.seq_len_4h
        return {
            "features_4h": torch.from_numpy(np.array(self.features_4h[i - L + 1 : i + 1])).unsqueeze(0),
...
def set_model1_outputs(npz_path, m1_outputs):
    ...
    assert m1_outputs.shape == expected_shape, f"shape mismatch {m1_outputs.shape} vs {expected_shape}"
    d["model1_outputs"] = m1_outputs.astype(np.float32)
    np.savez_compressed(npz_path, **d)


if __name__ == "__main__":
    m1 = Model1Dataset(os.path.join(BASE_DIR, "model1_dataset.npz"), window = 192)
    print(f"Model1  T={m1.T}  F_node={m1.F_node}  F_global={m1.F_global}  F_btc_raw={m1.F_btc_raw}")
    m2 = Model2Dataset(os.path.join(BASE_DIR, "model2_dataset.npz"), seq_len_4h = 72, seq_k = 8, split = "all", stride = 4, lookback_15m = 96, lookback_1h = 48)
    print(f"Model2  T_4h={m2.T_4h}  T_1h={m2.T_1h}  T_15m={m2.T_15m}")
    print(f"Model2  F_4h={m2.F_4h}  F_1h={m2.F_1h}  F_15m={m2.F_15m}  F_model1={m2.F_model1}")
